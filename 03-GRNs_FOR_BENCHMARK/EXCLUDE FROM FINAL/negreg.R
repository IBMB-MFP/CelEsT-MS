...
negative_regulators <- unique(c(str_remove(Wormbase_negative_regulators$gseq, "[a-z]{1}$"), Uniprot_negative_regulators))

combo3_GRN <- read.table("output/GRNs/allthree_equalweights.txt",
                         header = TRUE)

combo3_GRN[combo3_GRN$source %in% negative_regulators, "weight"] <- -combo3_GRN[combo3_GRN$source %in% negative_regulators, "weight"]

write.table(combo3_GRN,
            "output/GRNs/allthree_equalweights_negreg.txt",
            sep = "\t",
            row.names = FALSE,
            col.names = TRUE)
