...
TForth_1500 <- read.table("output/GRNs/TF_orthprobs_cut1500_fdr0.5.txt",
                               sep = "\t",
                               header = TRUE)
...
do.all.GRN.combinations(GRNlist = list(TForth_1500,
                                       ChIP_HOTexcl_nocut,
                                       eY1H_net),
                        prefix = "allthree_TForth",
                        weight_vec = c(0.2, 0.5, 0.3))

# do FIMO and MODern overlap restricted set with restricted observations for direct comparison

genes_present_in_ChIP_and_FIMO <- base::intersect(base::intersect(unique(TForth_1500$source), unique(ChIP_HOTexcl_nocut$source)), observations$target_gseq)

write.table(observations[observations$target_gseq %in% genes_present_in_ChIP_and_FIMO, ], 
            "output/TForthChIPnocutexcl_observations.txt",
            col.names = TRUE,
            row.names = TRUE,
            sep = "\t")
...
combine.GRNs(GRNlist = list(TForth_1500,
                            ChIP_HOTexcl_nocut),
             weighted = TRUE,
             filename = "TForthChIPnocutexcl_weighted")

combine.GRNs(GRNlist = list(TForth_1500,
                            ChIP_HOTexcl_nocut),
             weighted = FALSE,
             filename = "TForthChIPnocutexcl_noweights")
