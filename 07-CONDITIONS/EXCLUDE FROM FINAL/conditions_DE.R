...
HS_experiments <- unlist(sapply(c(HS_SRA$control, HS_SRA$treatment), strsplit, split = ", "))

#### PREPARE COUNTS FOR HS SET (SRA EXPERIMENTS) ####

FCfilelist <- list.files("input/conditions_counts/")

HSfiles <- FCfilelist[str_remove(FCfilelist, '_FCcounts.txt') %in% HS_experiments]

counts_list <- lapply(HSfiles, function(x){
...
colnames(counts_mat) <- str_remove(HSfiles, "_FCcounts.txt")
row.names(counts_mat) <- counts_list[[1]]$Geneid

#### RAW DE STATISTICS ####

allstudiesDE <- apply(HS_SRA, 1, function(thisstudy){
  
  ## For DEBUGGING
  # thisstudy <- unlist(HS_SRA[2, ])
...
names(t_values) <- HS_SRA$Accession
...
allstudiesDE_RAPToR <- apply(HS_SRA, 1, function(thisstudy){
  
  ## FOR DEBUGGING
  # thisstudy <- unlist(HS_SRA[1, ])
...
names(allstudiesDE_RAPToR) <- HS_SRA$Accession
...
row.names(HS_SRA) <- HS_SRA$Accession

write.table(allstudiesDE_RAPToR_df, 
            "output/HS_DE_RAPToR.txt",
            sep = "\t",
            row.names = TRUE,
            col.names = TRUE)
