...
chip_or_fimo_subset <- function(cutoff = 100,
  ...
  if(!is.null(TFlist)){
    targetlist_MALETFs <- targetlist[names(targetlist) %in% TFlist]
  } else {
    targetlist_MALETFs <- targetlist
  }
  
  targetlist_MALEcutoff <- reshape2::melt(lapply(targetlist_MALETFs, function(x){x[1:min(cutoff, length(x))]}))
  names(targetlist_MALEcutoff) <- c("target", "source")
  
  # remove autoregulation
  targetlist_MALEcutoff <- targetlist_MALEcutoff[!(targetlist_MALEcutoff$target == targetlist_MALEcutoff$source), ]
  ...
  targetlist_MALEcutoff[str_detect(targetlist_MALEcutoff$target, "^[0-9]"), "target"] <- paste0("X", targetlist_MALEcutoff[str_detect(targetlist_MALEcutoff$target, "^[0-9]"), "target"])
  
  targetlist_MALEcutoff[, "weight"] <- 1
  
  if(isTRUE(add_sign)){
    
    for(i in 1:length(negative_regulators)){
      
      targetlist_MALEcutoff[targetlist_MALEcutoff$TF == negative_regulators[i], "weight"] <- -1
      
    }
    
  }
  
  targetlist_MALEcutoff <- targetlist_MALEcutoff[, c("source", "target", "weight")]
  
  return(targetlist_MALEcutoff)
...
counts_to_tpm <- function(counts, featureLength, meanFragmentLength) {
  ...
  # Compute effective lengtMALE of features in each library.
...
MALE_experiments <- unlist(sapply(c(MALE_SRA$control, MALE_SRA$treatment), strsplit, split = ", "))

#### PREPARE COUNTS FOR MALE SET (SRA EXPERIMENTS) ####

FCfilelist <- list.files("input/conditions_counts/")

MALEfiles <- FCfilelist[str_remove(FCfilelist, '_FCcounts.txt') %in% MALE_experiments]

counts_list <- lapply(MALEfiles, function(x){
...
colnames(counts_mat) <- str_remove(MALEfiles, "_FCcounts.txt")
row.names(counts_mat) <- counts_list[[1]]$Geneid

#### RAW DE STATISTICS ####

allstudiesDE <- apply(MALE_SRA, 1, function(thisstudy){
  
  ## For DEBUGGING
  # thisstudy <- unlist(MALE_SRA[2, ])
...
names(t_values) <- MALE_SRA$Accession
...
allstudiesDE_RAPToR <- apply(MALE_SRA, 1, function(thisstudy){
  
  ## FOR DEBUGGING
  # thisstudy <- unlist(MALE_SRA[7, ])
  ...
  tryCatch({
    ...
    my_temp_lengtMALE <- sapply(row.names(thisdata), function(x){
      
      mean(wormRef::Cel_genes[wormRef::Cel_genes$wb_id == x, "transcript_length"])
      
    })
    
    my_temp_lengtMALE <- my_temp_lengtMALE[!is.na(my_temp_lengtMALE)]
    
    gene_there <- base::intersect(names(my_temp_lengtMALE), row.names(thisdata))
    
    thisdata_TPM <- counts_to_tpm(counts = thisdata[gene_there, ],
                                  featureLength = my_temp_lengtMALE[gene_there],
    ...
    wormRef_gene_lengtMALE <- sapply(row.names(interp_tpm), function(x){
    ...
    interp_count <- t((t (exp(interp_tpm) - 1)/ 1e6) * (libsize / median(wormRef_gene_lengtMALE))) * wormRef_gene_lengtMALE
...
names(allstudiesDE_RAPToR) <- MALE_SRA$Accession
