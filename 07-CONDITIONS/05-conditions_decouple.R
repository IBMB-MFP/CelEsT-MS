...
rcorr(as.matrix(HS_decouple[["decouple_z"]]))

# weird! 0.00 correlation? Yes. Wow.

cor.test(HS_decouple[["decouple_z"]]$GSE122015, HS_decouple[["decouple_z"]]$GSE162064 )

# exclude outlier sample; GSE122015
HS_sample_exclude <- c("GSE122015")
HS_decouple[["decouple_z"]] <- HS_decouple[["decouple_z"]][, !colnames(HS_decouple[["decouple_z"]]) %in% HS_sample_exclude]
...
HS_TF_DE_meanz <- HS_TF_DE_meanz[row.names(HS_decouple[["decouple_z"]])]

HS_meanz <- rowMeans(HS_decouple[["decouple_z"]])
...
saveRDS(HS_DE_vs_activity_plot, 
        "plotdata/HS_DE_vs_activity_plot.rds")
...
HS_decouple_mlm_wide_p <- HS_decouple$decouple_p[, !colnames(HS_decouple$decouple_p) %in% HS_sample_exclude]

HS_decouple_mlm_wide_p <- data.frame(HS_decouple_mlm_wide_p)
...
HS_forbubbleplot[abs(HS_forbubbleplot$p_val) < 3, "label"] <- ""

saveRDS(HS_forbubbleplot, "plotdata/HS_forbubbleplot.rds")
# HS_forbubbleplot <- readRDS("plotdata/HS_forbubbleplot.rds")

HScolours_for_plot <- map2color(HS_forbubbleplot$p_val, pal = colorRampPalette(c("grey", "grey", "grey", "red", "red", "red"))(100))
...
ggplot(aes(x = mean_z, y = p_val, size = 2^p_val, label = toupper(label)), data = HS_forbubbleplot) + 
  geom_point(col = HScolours_for_plot, alpha = 0.7) + 
  theme_classic() + 
...
pdf("graphics/HS_bubbleplot_FORCARTOON.pdf",
    height = 2.5,
    width = 2.5)
...
MALE_decouple <- meta.analysis.decouple(DEdata = MALE_DE,
                                      use_network = CelEsT)
pdf("graphics/MALE_samples_heatmap.pdf",
    height = 4, 
    width = 4)

heatmap.2(as.matrix(MALE_decouple[["decouple_z"]]),
          col = mainpal,
          dendrogram = "column",
          density = "none",
          key = FALSE,
...
rcorr(as.matrix(MALE_decouple[["decouple_z"]]))

# exclude outlier sample; GSE222447
MALE_sample_exclude <- c("GSE222447")

MALE_decouple[["decouple_z"]] <- MALE_decouple[["decouple_z"]][, !colnames(MALE_decouple[["decouple_z"]]) %in% MALE_sample_exclude]
...
MALE_TF_DE_meanz <- MALE_TF_DE_meanz[row.names(MALE_decouple[["decouple_z"]])]

MALE_meanz <- rowMeans(MALE_decouple[["decouple_z"]])
...
MALE_DE_vs_activity_plot[abs(MALE_DE_vs_activity_plot$DE) < 1.96 & abs(MALE_DE_vs_activity_plot$act)  < 1.96, "label"] <- ""

saveRDS(MALE_DE_vs_activity_plot,
        "plotdata/MALE_DE_vs_activity_plot.rds")
...
MALE_decouple_mlm_wide_p <- MALE_decouple$decouple_p[, !colnames(MALE_decouple$decouple_p) %in% MALE_sample_exclude]

MALE_decouple_mlm_wide_p <- data.frame(MALE_decouple_mlm_wide_p)
...
MALE_forbubbleplot[abs(MALE_forbubbleplot$p_val) < 3, "label"] <- ""

saveRDS(MALE_forbubbleplot, "plotdata/MALE_forbubbleplot.rds")
# MALE_forbubbleplot <- readRDS("plotdata/MALE_forbubbleplot.rds")

MALEcolours_for_plot <- map2color(MALE_forbubbleplot$p_val, pal = colorRampPalette(c("grey", "grey", "grey", "red", "red", "red"))(100))
...
ggplot(aes(x = mean_z, y = p_val, size = 2^p_val, label = toupper(label)), data = MALE_forbubbleplot) + 
  geom_point(col = MALEcolours_for_plot, alpha = 0.7) + 
  theme_classic() + 
  geom_label_repel(size = 1.8, max.overlaps = 40,
                   label.padding = 0.1) +
  geom_vline(xintercept = 0, linetype = "dashed", col = "grey") + 
...
PA14_decouple <- meta.analysis.decouple(DEdata = PA14_DE,
                                        use_network = CelEsT)

heatmap.2(as.matrix(PA14_decouple[["decouple_z"]]),
          col = mainpal,
          dendrogram = "column",
          density = "none",
          key = FALSE,
...
rcorr(as.matrix(PA14_decouple[["decouple_z"]]))

## there are two that are obviously the same dataset
# so exclude GSE146406
PA14_sample_exclude <- c("GSE146406")

pdf("graphics/PA14_samples_heatmap.pdf",
    height = 4, 
    width = 4)

heatmap.2(as.matrix(PA14_decouple[["decouple_z"]])[, !colnames(as.matrix(PA14_decouple[["decouple_z"]])) %in% PA14_sample_exclude],
          col = mainpal,
          dendrogram = "column",
          density = "none",
          key = FALSE,
...
pdf("graphics/PA14_samples_heatmap_LEGEND.pdf")
...
PA14_DE_zscores <- data.frame(matrix(nrow = nrow(PA14_DE), ncol = ncol(PA14_DE)))

row.names(PA14_DE_zscores) <- row.names(PA14_DE)
colnames(PA14_DE_zscores) <- colnames(PA14_DE)

PA14_DE_zscores[] <- lapply(PA14_DE, function(x){
...
PA14_TF_DE_meanz <- PA14_TF_DE_meanz[row.names(PA14_decouple[["decouple_z"]])]

PA14_meanz <- rowMeans(PA14_decouple[["decouple_z"]])
...
PA14_DE_vs_activity_plot[abs(PA14_DE_vs_activity_plot$DE) < 1.96 & abs(PA14_DE_vs_activity_plot$act)  < 1.96, "label"] <- ""

saveRDS(PA14_DE_vs_activity_plot, 
         "plotdata/PA14_DE_vs_activity_plot.rds")

cor.test(PA14_DE_vs_activity_plot$DE,
         PA14_DE_vs_activity_plot$act)
...
PA14_decouple_mlm_wide_p <- PA14_decouple$decouple_p[, !colnames(PA14_decouple$decouple_p) %in% PA14_sample_exclude]

PA14_decouple_mlm_wide_p <- data.frame(PA14_decouple_mlm_wide_p)

PA14_decouple_mlm_wide_p[] <- lapply(PA14_decouple_mlm_wide_p, as.numeric)

PA14_decouple_mlm_wide_pgeomean <- apply(PA14_decouple_mlm_wide_p, 1, gm_mean)

PA14_decouple_mlm_wide_pgeomean[order(PA14_decouple_mlm_wide_pgeomean)]
...
PA14_forbubbleplot[abs(PA14_forbubbleplot$p_val) < 5, "label"] <- ""

saveRDS(PA14_forbubbleplot, "plotdata/PA14_forbubbleplot.rds")
# PA14_forbubbleplot <- readRDS("plotdata/PA14_forbubbleplot.rds")

PA14colours_for_plot <- map2color(PA14_forbubbleplot$p_val, pal = colorRampPalette(c("grey", "grey", "grey", "red", "red", "red"))(100))
...
ggplot(aes(x = mean_z, y = p_val, size = 2^p_val, label = toupper(label)), data = PA14_forbubbleplot) + 
  geom_point(col = PA14colours_for_plot, alpha = 0.7) + 
  theme_classic() + 
  geom_label_repel(size = 1.8, max.overlaps = 40,
                   label.padding = 0.1) +
  geom_vline(xintercept = 0, linetype = "dashed", col = "grey") + 
