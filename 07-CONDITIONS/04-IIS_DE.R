...
chip_or_fimo_subset <- function(cutoff = 100,
  ...
  if(!is.null(TFlist)){
    targetlist_HSTFs <- targetlist[names(targetlist) %in% TFlist]
  } else {
    targetlist_HSTFs <- targetlist
  }
  
  targetlist_HScutoff <- reshape2::melt(lapply(targetlist_HSTFs, function(x){x[1:min(cutoff, length(x))]}))
  names(targetlist_HScutoff) <- c("target", "source")
  
  # remove autoregulation
  targetlist_HScutoff <- targetlist_HScutoff[!(targetlist_HScutoff$target == targetlist_HScutoff$source), ]
  ...
  targetlist_HScutoff[str_detect(targetlist_HScutoff$target, "^[0-9]"), "target"] <- paste0("X", targetlist_HScutoff[str_detect(targetlist_HScutoff$target, "^[0-9]"), "target"])
  
  targetlist_HScutoff[, "weight"] <- 1
  
  if(isTRUE(add_sign)){
    
    for(i in 1:length(negative_regulators)){
      
      targetlist_HScutoff[targetlist_HScutoff$TF == negative_regulators[i], "weight"] <- -1
      
    }
    
  }
  
  targetlist_HScutoff <- targetlist_HScutoff[, c("source", "target", "weight")]
  
  return(targetlist_HScutoff)
...
IIS_experiments <- unlist(sapply(c(all_SRA$control, all_SRA$treatment), strsplit, split = ", "))

#### PREPARE COUNTS FOR IIS SET (SRA EXPERIMENTS) ####

FCfilelist <- list.files("input/IIS_counts/")

IISfiles <- FCfilelist[str_remove(FCfilelist, '_FCcounts.txt') %in% IIS_experiments]

counts_list <- lapply(IISfiles, function(x){
  
  read.table(paste0("input/IIS_counts/", x),
             sep = "\t",
             skip = 1,
             header = TRUE)
...
colnames(counts_mat) <- str_remove(IISfiles, "_FCcounts.txt")
...
names(t_values) <- paste0(all_SRA$Accession, all_SRA$genotype)
...
allstudiesDE_RAPToR <- apply(all_SRA[nrow(all_SRA), ], 1, function(thisstudy){
  ...
  tryCatch({
    ...
    my_temp_lengtIIS <- sapply(row.names(thisdata), function(x){
      
      mean(wormRef::Cel_genes[wormRef::Cel_genes$wb_id == x, "transcript_length"])
      
    })
    
    my_temp_lengtIIS <- my_temp_lengtIIS[!is.na(my_temp_lengtIIS)]
    
    gene_there <- base::intersect(names(my_temp_lengtIIS), row.names(thisdata))
    
    thisdata_TPM <- counts_to_tpm(counts = thisdata[gene_there, ],
                                  featureLength = my_temp_lengtIIS[gene_there],
    ...
    wormRef_gene_lengtIIS <- sapply(row.names(interp_tpm), function(x){
    ...
    interp_count <- t((t (exp(interp_tpm) - 1)/ 1e6) * (libsize / median(wormRef_gene_lengtIIS))) * wormRef_gene_lengtIIS
    ...
    # allstudiesDE_RAPToR[[19]] <- results(dd1, contrast = c("strain", "treatment", "control"))
  }, error = function(e){
...
names(allstudiesDE_RAPToR) <- paste0(all_SRA$Accession, all_SRA$genotype)
...
allstudies_genenames <- genenameBM[match(row.names(allstudiesDE_RAPToR_df), genenameBM$wbps_gene_id), "wormbase_gseq"]

allstudiesDE_RAPToR_df <- allstudiesDE_RAPToR_df[!(duplicated(allstudies_genenames)|duplicated(allstudies_genenames, fromLast = TRUE)), ]
allstudies_genenames <- allstudies_genenames[!(duplicated(allstudies_genenames)|duplicated(allstudies_genenames, fromLast = TRUE))]

rownames(allstudiesDE_RAPToR_df) <- allstudies_genenames

write.table(allstudiesDE_RAPToR_df, 
            "output/IIS_DE_RAPToR.txt",
            sep = "\t",
            row.names = TRUE,
            col.names = TRUE)
