...
maxCelEsT = pd.read_csv("output/GRNs/orthCelEsTMAXcov_equalweights.txt",
                  sep = '\t')
...
#%%
maxCelEsT_df = dc.benchmark(benchRAPToR, obs1, FIMOnets, perturb='target_gseq', sign=-1, verbose=True, decouple_kws = decouple_kws)

dc.plot_metrics_scatter_cols(maxCelEsT_df, col = 'method', figsize = (9, 5), groupby = 'net')

maxCelEsT_df.to_csv("output/benchmark_out/maxCelEsT_df.tsv", sep='\t', index=False)  

#%%

for key in FIMOnets:
     
    maxCelEsT_raptor_shuffle_stats = []
    ...
    for i in range(1, 101):
        ...
        maxCelEsT_raptor_shuffle_stats.append(my_selection)

    TFmax_shufflestat_raptor = pd.concat(maxCelEsT_raptor_shuffle_stats, ignore_index = True)

    TFmax_shufflestat_raptor.to_csv("output/benchmark_out/" + str(key) + "_benchRAPToR_shufflestats.tsv", sep = '\t', index=False) 
