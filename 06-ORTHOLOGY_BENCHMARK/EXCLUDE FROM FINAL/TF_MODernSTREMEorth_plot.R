...
make.TF.orth.GRN <- function(cutoff,
  ...
  write.table(tempGRN,
              file = paste0("output/GRNs/TF_MODernSTREMEorthprobs_cut", cutoff, "_fdr", fdrcut, suffix,".txt"),
              sep = "\t",
              row.names = FALSE,
              col.names = TRUE)
...
TForthprobs_files <- bench_out_filelist[str_detect(bench_out_filelist, "TF_MODernSTREMEorth")]
TForthprobs_GRNs <- GRN_filelist[str_detect(GRN_filelist, "TF_MODernSTREMEorth")]
...
TFnumbers <- sapply(cutoffs_vec, function(thiscutoff){
  ...
  sapply(fdr_vec, function(thisfdr){
    ...
    tempGRN <- read.table(paste0("output/GRNs/TF_MODernSTREMEorthprobs_cut", thiscutoff, "_fdr", thisfdr, ".txt"),
                          header = TRUE)
...
pdf(paste0("graphics/TF_MODernSTREMEorth_methodsplot_fdr", fdr_vec[i], ".pdf"),
    width = 5,
    height = 4)
...
for(i in 1:length(cutoffs_vec)){
  ...
  pdf(paste0("graphics/TF_MODernSTREMEorth_methodsplot_fdr", cutoffs_vec[i], ".pdf"),
      width = 5,
      height = 4)
...
TF_auroc_mlm <- sapply(cutoffs_vec, function(thiscutoff){
  ...
  sapply(fdr_vec, function(thisfdr){
    ...
    tempbench <- read.table(paste0("output/benchmark_out/TF_MODernSTREMEorthprobs_cut", thiscutoff, "_fdr", thisfdr, "_benchRAPToR.tsv"),
                          sep = "\t",
                          header = TRUE)
...
?redblue
colors = seq(0.7, 0.8, length.out=101)
redblue(100)
...
auroccolors = seq(0.6, 0.7, length.out=101)
...
TForth_benchraptor_shufflestat_files <- c(paste0(c("TForth", "CelEsT", "control", "FIMOfull"), "_benchRAPToR_shufflestats.tsv"))
...
looky <- read.table("output/GRNs/TF_MODernSTREMEorthprobs_cut5000_fdr0.8.txt",
                    header = TRUE)

hist(table(looky$source))

names(allTFs_labelused[match(names(sapply(allTFS_manualtoptargets_HOTexcl, length)[sapply(allTFS_manualtoptargets_HOTexcl, length) > 300]), allTFs_labelused)])


trytry <- cbind(sapply(allTFS_manualtoptargets_HOTexcl, length)[sapply(allTFS_manualtoptargets_HOTexcl, length) > 300],
      table(looky$source)[names(allTFs_labelused[match(names(sapply(allTFS_manualtoptargets_HOTexcl, length)[sapply(allTFS_manualtoptargets_HOTexcl, length) > 300]), allTFs_labelused)])])

mean(trytry[, 2] / trytry[, 1])

prolificgenes <- names(allTFs_labelused[match(names(sapply(allTFS_manualtoptargets_HOTexcl, length)[sapply(allTFS_manualtoptargets_HOTexcl, length) > 300]), allTFs_labelused)])

observations <- read.table("output/benchmark_observations.txt")
observations$target_gseq                                              

sum(prolificgenes %in% observations$target_gseq)
