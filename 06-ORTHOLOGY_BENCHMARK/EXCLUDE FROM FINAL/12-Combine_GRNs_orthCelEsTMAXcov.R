...
ChIPorth500 <- ChIPorth500[ChIPorth500$source != "T23D8.8", ]
...
orthCelEsT <- read.table("output/GRNs/orthCelEsTMAXcov_equalweights.txt", header = TRUE)
...
interactions_present <- sapply(list(FIMO1500,
                                    ChIPorth500,
                                    eY1H_net), function(x){     
...
commonTFs <- base::intersect(base::intersect(unique(eY1H_net$source), unique(ChIPorth500$source)), unique(FIMO1500$source))

orthCelEsT_common <- orthCelEsT[orthCelEsT$source %in% commonTFs, ]

common_interactions_present <- sapply(list(FIMO1500,
                                           ChIPorth500,
                                           eY1H_net), function(x){     
