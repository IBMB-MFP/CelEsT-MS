...
make.TF.orth.GRN <- function(cutoff,
  ...
  tempGRN <- do.call(rbind, lapply(names(TF_orthology_probs), function(thisTF){
    ...
    elegans_x <- elegans_targets[, thisTF]
    names(elegans_x) <- row.names(elegans_targets)
...
#### also try ordering by TF orth probs and doing simple cutoffs

# do full targets (so NOT excluding briggsae operons as potential targets in elegans, if they are not downstream in operons in elegans)
# NB they are still excluded for the purposes of calculating orthology scores

elegans_targets_nobrigexcl <- unique(elegans_FIMO_censored$sequence_name)[!unique(elegans_FIMO_censored$sequence_name) %in% downstream_operon_genes]

elegans_target_table_nobrigexcl <- as.data.frame(lapply(calc_score_by_motif, function(thismotif_hits){
  
  thismotif_hits <- thismotif_hits[names(thismotif_hits) %in% elegans_targets_nobrigexcl]
  tempvec <- vector(length = length(elegans_targets_nobrigexcl))
  names(tempvec) <- elegans_targets_nobrigexcl
  tempvec[] <- 0
...
make.TF.orth.GRN.alternative <- function(cutoff = 1000,
                             TF_orthology_prob_list = TF_orthology_probs,
                             elegans_targets = elegans_target_table_nobrigexcl,
                             suffix = NULL,
                             mintargets = 15){
  # cutoff = 1500
  # fdrcut = 0.5
  tempGRN <- do.call(rbind, lapply(names(TF_orthology_prob_list), function(thisTF){
    ...
    elegans_df <- data.frame("target" = names(elegans_x_order),
                             "score" = elegans_x_order)
    
    elegans_df[, "orth_probs"] <- x[names(elegans_x_order)]
    
    elegans_df[is.na(elegans_df$orth_probs), "orth_probs"] <- 1
    
    elegans_df_cens <- elegans_df[elegans_df$score > 0, ]
    
    elegans_df_cens <- elegans_df_cens[order(elegans_df_cens$orth_probs), ]
    
    thiscutoff <- min(cutoff, nrow(elegans_df_cens))
    
    cut_names <- elegans_df_cens[1:thiscutoff, "target"]

    data.frame("source" = rep(thisTF, times = length(cut_names)),
               "target" = cut_names,
               "weight" = rep(1, times = length(cut_names)))
    
  }))

  # names(TF_orthology_prob_list)[!names(TF_orthology_prob_list) %in% unique(tempGRN$source)]
  
  tempGRN <- tempGRN[!tempGRN$source %in% names(table(tempGRN$source))[table(tempGRN$source) < mintargets], ]
  
  write.table(tempGRN,
              file = paste0("output/GRNs/TF_orthprobs_cut", cutoff, "_", suffix,".txt"),
              sep = "\t",
              row.names = FALSE,
              col.names = TRUE)
...
for(i in 1:length(cutoffs_vec)){
    ...
    make.TF.orth.GRN.alternative(cutoff = cutoffs_vec[i],
                     TF_orthology_prob_list = TF_orthology_probs,
                     elegans_targets = elegans_target_table_nobrigexcl,
                     suffix = "ALTERNATIVE")
...
#### after run I read through it

bench_out_filelist <- list.files("output/benchmark_out/")

TForthALT_benchraptor_files <- bench_out_filelist[str_detect(bench_out_filelist, "benchRAPToR") & str_detect(bench_out_filelist, "TF_orthprobsALT")]

TForthALT_benchraptor_list <- lapply(TForthALT_benchraptor_files, function(x){
...
TForthALT_benchraptor_all <- do.call(rbind, TForthALT_benchraptor_list)

TForthALT_benchraptor_allmethods_plot <- TForthALT_benchraptor_all[TForthALT_benchraptor_all$metric %in% c("auroc", "auprc"), c("net", "method", "metric", "score")]
TForthALT_benchraptor_allmethods_plot <- tidyr::pivot_wider(TForthALT_benchraptor_allmethods_plot, names_from = c("metric"), values_from = "score")

# TForthALT_benchraptor_allmethods_plot[, "label"] <- as.character(TForthALT_benchraptor_allmethods_plot$net)
# TForthALT_benchraptor_allmethods_plot[str_detect(unlist(TForthALT_benchraptor_allmethods_plot$net), "shuffle"), "label"] <- ""

TForthALT_benchraptor_allmethods_plot <- TForthALT_benchraptor_allmethods_plot[TForthALT_benchraptor_allmethods_plot$method == "mlm_estimate", ]

FIMO_nohomo_benchraptor_allmethods_plot <- readRDS("plotdata/FIMO_nohomo_benchraptor_allmethods_plot.rds")
FIMO_nohomo_benchraptor_allmethods_plot <- FIMO_nohomo_benchraptor_allmethods_plot[FIMO_nohomo_benchraptor_allmethods_plot$method == "mlm_estimate", ]

FIMO_nohomo_benchraptor_allmethods_plot <- FIMO_nohomo_benchraptor_allmethods_plot[, colnames(FIMO_nohomo_benchraptor_allmethods_plot) %in% colnames(TForthALT_benchraptor_allmethods_plot)]

FIMO_nohomo_benchraptor_allmethods_plot[, "order"] <- "ChIPscore"
TForthALT_benchraptor_allmethods_plot[, "order"] <- "orth"

both_nohomo_benchraptor_allmethods_plot <- rbind(FIMO_nohomo_benchraptor_allmethods_plot,
                                                  TForthALT_benchraptor_allmethods_plot)

ggplot(data = both_nohomo_benchraptor_allmethods_plot, aes(x = as.numeric(net), y = auprc, colour = order)) + 
  geom_point() + 
  geom_line() + 
...
ggplot(data = both_nohomo_benchraptor_allmethods_plot, aes(x = as.numeric(net), y = auroc, colour = order)) + 
  geom_point() + 
  geom_line() +
  scale_x_log10(breaks = c(100, 500, 1000, 1500, 2000, 2500, 3000, 5000)) +
  ylab("Precision (AUPRC)") +
