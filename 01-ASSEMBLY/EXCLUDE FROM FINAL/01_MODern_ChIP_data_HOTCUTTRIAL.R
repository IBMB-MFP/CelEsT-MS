...
allmodERN_TFs_BM[!allmodERN_TFs_BM$wormbase_gseq %in% MODern_allTFs_data$Sequence_name, ]

allmodERN_TFsONLY_BM <- allmodERN_TFs_BM[allmodERN_TFs_BM$wormbase_gseq %in% MODern_allTFs_data$Sequence_name, ]
...
names(allTFs_labelused)[1:310] <- allmodERN_TFs_BM[match(allTFs_labelused[1:310], allmodERN_TFs_BM$wormbase_locus), "wormbase_gseq"]
...
# 356 TFs in MODern all TFs list and MODern/ENCODE/literature
...
# HOT exclusion leads to 46 TFs with fewer than 15 targets
...
intersect(unique(unlist(allTFS_manualtoptargets_HOTexcl[!(sapply(allTFS_manualtoptargets_HOTexcl, length) < 15)])), unique(unlist(allTFS_manualtoptargets)))

nrow(unique(mcols(chip_peaks_overlap)["manual_target"]))
